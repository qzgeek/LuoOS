...
HEOS 是为 HE 服务器维护的一组 Minecraft 服务端功能集合。它同时提供 Fabric 模组和 Folia 插件版本，目标是把登录认证、封禁、白名单、玩家数据迁移、TPS 显示、配方同步和若干服务端修复放在同一个轻量工具里。
...
- 支持正版玩家和离线玩家分离存储，避免同名正版账号与离线账号覆盖登录数据。
